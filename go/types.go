// Package costliving provides a Go client for the Cost of Living API.
//
// For more information, visit: https://apiverve.com/marketplace/costliving?utm_source=go&utm_medium=readme
package costliving
...
// Request contains the parameters for the Cost of Living API.
//
// Parameters:
//   - location: string - State name, state code (e.g., CA, NY), or major city name. Omit for all regions ranked.
//   - compare: string - Second location for salary comparison calculation
type Request struct {
	Location string `json:"location,omitempty"` // Optional
	Compare string `json:"compare,omitempty"` // Optional
}
...
// ResponseData contains the data returned by the Cost of Living API.
...
// FromData represents the from object.
type FromData struct {
	SearchedLocation string `json:"searchedLocation"`
	Region string `json:"region"`
	RegionName string `json:"regionName"`
	CostIndex int `json:"costIndex"`
}

// ToData represents the to object.
type ToData struct {
	SearchedLocation string `json:"searchedLocation"`
	Region string `json:"region"`
	RegionName string `json:"regionName"`
...
// ComparisonData represents the comparison object.
type ComparisonData struct {
	CostDifference float64 `json:"costDifference"`
	Direction string `json:"direction"`
	SalaryEquivalent SalaryEquivalentData `json:"salaryEquivalent"`
}

// SalaryEquivalentData represents the salaryEquivalent object.
type SalaryEquivalentData struct {
	Description string `json:"description"`
	FromSalary int `json:"fromSalary"`
	EquivalentSalary int `json:"equivalentSalary"`
}
