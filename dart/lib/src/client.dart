...
class CostlivingValidationException implements Exception {
  final List<String> errors;

  CostlivingValidationException(this.errors);

  @override
  String toString() => 'CostlivingValidationException: ${errors.join("; ")}';
}
...
/// Cost of Living API client.
///
/// For more information, visit: https://apiverve.com/marketplace/costliving?utm_source=dart&utm_medium=readme
///
/// Parameters:
/// * [location] - State name, state code (e.g., CA, NY), or major city name. Omit for all regions ranked.
/// * [compare] - Second location for salary comparison calculation
class CostlivingClient {
  final String apiKey;
  ...
  static final Map<String, ValidationRule> _validationRules = <String, ValidationRule>{
    'location': ValidationRule(type: 'string', required: false),
    'compare': ValidationRule(type: 'string', required: false),
  };

  CostlivingClient(this.apiKey, {
    this.baseUrl = 'https://api.apiverve.com/v1/costliving',
    http.Client? httpClient,
  }) : _httpClient = httpClient ?? http.Client();

  /// Validates parameters against defined rules.
  /// Throws [CostlivingValidationException] if validation fails.
  void _validateParams(Map<String, dynamic> params) {
    ...
    if (errors.isNotEmpty) {
      throw CostlivingValidationException(errors);
    }
  }

  /// Execute a request to the Cost of Living API.
  ///
  /// Parameters are validated before sending the request.
  Future<CostlivingResponse> execute(Map<String, dynamic> params) async {
    // Validate parameters
    _validateParams(params);
    if (apiKey.isEmpty) {
      throw CostlivingException('API key is required. Get your API key at: https://apiverve.com');
    }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        return CostlivingResponse.fromJson(json);
      } else if (response.statusCode == 401) {
        throw CostlivingException('Invalid API key');
      } else if (response.statusCode == 404) {
        throw CostlivingException('Resource not found');
      } else {
        throw CostlivingException('API error: ${response.statusCode}');
      }
    } catch (e) {
      if (e is CostlivingException) rethrow;
      throw CostlivingException('Request failed: $e');
    }
  }
...
/// Exception thrown by the Cost of Living API client.
class CostlivingException implements Exception {
  final String message;

  CostlivingException(this.message);

  @override
  String toString() => 'CostlivingException: $message';
}
